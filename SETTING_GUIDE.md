...
| **OpenAI API 키** | — | 아래 2번 참고 |
| **인터넷 연결** | — | 게임 중 대화마다 OpenAI API를 호출하므로 필수 |
...
1. [OpenAI API keys](https://platform.openai.com/api-keys) 접속 → 로그인
2. **Create new secret key** 클릭 → 생성된 키(`sk-...`) 복사 (창을 닫으면 다시 볼 수 없음)
3. 팀 공용 키를 쓰는 경우엔 팀원에게 키를 받는다
...
만들어진 `.env.local`을 메모장/VS Code로 열고, `your_openai_api_key_here` 부분을 실제 키로 바꾼다.
...
- 선불 크레딧이 $0이 되면 대답이 멈추므로, 당일 전에
  [OpenAI Billing](https://platform.openai.com/settings/organization/billing/overview)에서 잔액을 확인해 둔다.
...
| 잘 되다가 갑자기 대답이 안 옴 | 크레딧 소진/한도 초과 가능성 → 터미널의 `OpenAI API error` 로그 확인, 크레딧 충전 또는 다른 키로 교체 |
