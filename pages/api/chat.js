...
const INTENTS = ["normal", "date_request", "affectionate", "keyword_hit"];
...
- 유저가 먼저 데이트를 신청하면 당황하거나 부담스러워하며 거절하는 대사를 하고,
  love_delta는 반드시 0(변화 없음)으로 반환한다. 대화는 계속 이어간다.
...
- +5: 일기장 항목과 무관한, 외모/성격/행동/순간 중 조금이라도 구체적인 부분을
  짚어주는 칭찬. 완전히 뭉뚱그린 한 단어짜리 칭찬("예쁘다", "귀엽다" + 의미없는 말)만 아니면
  대체로 +5로 판단한다 (0보다 +5 쪽으로 후하게 판단할 것. 대신 꼭 칭찬이어야함).
...
2. love_delta가 +5이고 현재 호감도 N이 30 이상이면 → shy (설레고 부끄러워하는 반응)
3. love_delta가 +5이고 현재 호감도 N이 30 미만이면 → happy (순수하게 기뻐하는 반응)
4. love_delta가 0이면 → normal
5. love_delta가 -10이고 현재 호감도 N이 30 이하이면 → angry (진짜로 화내는 반응)
6. love_delta가 -10이고 현재 호감도 N이 31~39 사이이면 → normal (이미 충분히 친해져서 화내기보다 담담하게 받아침)
7. love_delta가 -10이고 현재 호감도 N이 40 이상이면 → sad (많이 좋아하게 된 상태라 오히려 서운하고 속상해하는 반응)
- shy는 반드시 1, 2번 조건에서만, angry는 반드시 5번 조건에서만 사용하고 그 외에는 절대 사용하지 않는다.
- reply의 말투와 감정선은 위에서 정해진 emotion과 반드시 일치해야 한다.

[user_intent - 반드시 아래 중 하나]
normal, date_request, affectionate, keyword_hit
...
function resolveEmotion(currentLove, loveDelta) {
  if (loveDelta === 10) {
    return "shy";
  }
  if (loveDelta === 5) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const emotion = resolveEmotion(currentLove, love_delta);
    const reply =
